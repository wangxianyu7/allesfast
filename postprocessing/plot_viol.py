"""
Pipeline to analyze allesfast output for planet transit timings

argument 1: allesfast path
...
import allesfast
import allesfast.postprocessing.plot_viol
...
def plot_viol(pathbase, liststrgstar, liststrgruns, lablstrgruns, pathimag, pvalthrs=1e-3):
    ...
    print('allesfast postprocessing violin plot started at %s...' % strgtimestmp)
    ...
    # read parameter keys, labels and posterior from allesfast output
    ...
    for i in gdat.indxruns:
        for m in gdat.indxstar:
            ...
            print('pathalle')
            print(pathalle)
            gdat.listobjtalle[i][m] = allesfast.allesclass(pathalle)
